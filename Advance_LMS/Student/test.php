...
<head>
  ...
  <style>
    svg path {
      ...
      stroke: #666; /* outline color */
      stroke-width: 1;
    }
    svg path:hover {
      fill: #f00; /* color on hover */
    }
    .district-label {
...
<br/>
  ...
  <div class="col-12 col-md-8">
        <h1>Welcome to Resource Hub</h1><br>
        <h2 class="text-2xl font-semibold mb-4 d-flex align-items-center">
            <span class="mr-2">📍</span> Choose a district on the map
        </h2>

        <div>
            <p style="display: inline-block; margin-right: 10px; color:#f00;">Result will show your Selected district + online Courses</p>
           
        </div><br>

        <h3>You are looking for an online course?</h3><br>
        <a href="resource_hub.php?district=<?php echo 'online'; ?>" class="text-blue-600 hover:underline mt-4"><button class="online">Online Courses</button></a>
        </div>
    <div class="col-12 col-md-4 mt-6 mt-md-0">
    <svg baseprofile="tiny" fill="#7c7c7c"  stroke="#ffffff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" version="1.2" viewbox="0 0 1000 1745"  xmlns="http://www.w3.org/2000/svg">
 ...
 <circle cx="149.2" cy="439.8" id="2">
  ...
  <div class="row justify-content-center mt-6">
    <button class="bg-yellow-500 text-white px-6 py-2 rounded-lg shadow-md hover:bg-yellow-600">
      ✉️ Post Ad
    </button>
  </div>
</div>
...
<script>
  document.querySelectorAll('svg path').forEach(item => {
    item.addEventListener('mouseenter', event => {
      ...
      const districtName = event.target.getAttribute('name');
    ...
    item.addEventListener('mouseleave', event => {
      // Remove the label when mouse leaves
      const label = document.querySelector('.district-label');
      if (label) {
        label.remove();
      }
    });
    ...
    item.addEventListener('click', event => {
      ...
      // Redirect to home.php with the district name as a query parameter
      window.location.href = `home.php?district=${districtName}`;
    });
  });
</script>
