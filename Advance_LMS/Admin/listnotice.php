            ...
            <div class="row page-titles mx-0">
                <div class="col p-md-0">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><a href="javascript:void(0)">List of Notice</a></li>
                    </ol>
                </div>
            </div>
            <!-- row -->
...
<script>
    $j(document).ready(function() {
        ...
        var table = $j('#noticeTable').DataTable({
            "ajax": "noticedata.php", // Path to your PHP file
            "columns": [
                { "data": "notice_id" },
                { "data": "notice_type" },
                { "data": "title" },
                { "data": "end_date" },
                { "data": "status" },
                { "data": null, "defaultContent": "<button class='edit-btn btn btn-primary'>Edit</button> &nbsp;&nbsp; <button class='delete-btn btn btn-danger'>Delete</button>" }
            ],
            "createdRow": function (row, data, dataIndex) {
                $j(row).find('.edit-btn').on('click', function () {
                    // Access the username from the current row's data object
                    var c_id = data.notice_id;
                    // Perform edit action with the username
                    window.location.href = 'edit_notice.php?id=' + c_id;
                });
